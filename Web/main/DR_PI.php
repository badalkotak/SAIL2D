...
if($user=="")
{
  ...
  $check=mysqli_query($con,"SELECT COUNT(ID) FROM PIDisagrement WHERE PostedBy='$postedby' && ReviewedBy='$reviewedby' && ScoreId='$id'");
  ...
  if($insert=mysqli_query($con,"INSERT INTO `PIDisagrement`(`ScoreId`, `PostedBy`, `ReviewedBy`) VALUES ('$id','$postedby','$reviewedby')"))
  {
   
    $query='Create table Chat_PI_'.$id.'_'.$postedby.'_'.$reviewedby. '( MsgId int AUTO_INCREMENT PRIMARY KEY, Message varchar(1000) not null, MsgTime TIMESTAMP, User int NOT NULL, FOREIGN KEY(User) REFERENCES UserDetails(UserId) )';
...
<body class="hold-transition skin-yellow-light sidebar-mini">
  <div class="wrapper">
            ...
            <div class="panel panel-primary">
                <div class="panel-heading" style="height: 80;">
                    ...
                    <?
                       if($userId==$postedby)
                        {	
                           echo "<form action='UpdateActivity.php?type=2&id=$id&ds=1' method=post><button class='btn btn-default pull-right'>Update Activity</button></form>";
                        }

                        else
                        {
                            echo "<form action=updatePIRating.php?id=$id method=post><button class='btn btn-default pull-right' name=id value=$id>Update Rating</button></form>";
                        }
                    ?>
                ...
                <div class="panel-body" id="chat">
                    ...
                    $query='SELECT * FROM Chat_PI_'.$id.'_'.$postedby.'_'.$reviewedby;
	...
	echo "<form action='agreeReview.php?type=2&id=$id&ds=1' method=post><button class='btn btn-Success'>Agree</button></form>";
?>
